...
void solve() {
    ...
    /*
    Each light maps to two switches
    If all the switches are off, all the lights are off
    Toggling a switch toggles the light

    For minimum, combine as many ones as possible
    For maximum, try splitting them up
    */
    int ones = accumulate(a.begin(), a.end(), 0);
    if (ones <= n) {
        cout << ones % 2 << " " << ones;
    } else {
        cout << ones % 2 << " " << n - (ones - n);
    }
}
