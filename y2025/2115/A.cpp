...
void solve() {
    ...
    /*
    We run a dp for each number and check for the min
    according to gcds
    */
    int g = a[0];
    for (int i = 1; i < n; i++) g = gcd(g, a[i]);

    int gcount = count(a.begin(), a.end(), g);
    if (gcount > 0) {
        cout << a.size() - gcount; return;
    }

    sort(a.begin(), a.end());
    int maxa = a[n - 1];

    vector<int> dp(maxa + 1, 100000);
    for (int x: a) dp[x] = 0;
    for (int i = maxa; i > 0; i--) {
        for (int j = 0; j < n; j++) {
            int gd = gcd(i, a[j]);
            dp[gd] = min(dp[gd], dp[i] + 1);
        }
    }
