...
void solve() {
    ...
    vector<int> values;
    for (auto [k, v]: cnt) values.push_back(v);
    sort(values.begin(), values.end());

    int i = 0;
    while (k > 0 && i < values.size() - 1) {
        int move = min(k, values[i]);
        values[i] -= move;
        k -= move;
        values[values.size() - 1] += move;
        i++;
    }

    int zeroes = 0;
    while (values[zeroes] == 0) zeroes++;
    cout << values.size() - zeroes;
}
